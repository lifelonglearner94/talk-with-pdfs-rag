...
class SimpleReranker:
    ...
    def score(self, query: str, docs: List[Document]) -> List[float]:
        ...
        for d in docs:
            ...
            overlap_score = unique_overlap + (overlap / len(dtoks)) * 0.01
            # Simple TF-IDF proxy: term frequency of query tokens normalized
            tfidf_proxy = sum(dtoks.count(t) for t in q_tokens) / len(dtoks)
            sc = self.overlap_weight * overlap_score + self.tfidf_weight * tfidf_proxy
            scores.append(sc)
            if self._cache_max:
